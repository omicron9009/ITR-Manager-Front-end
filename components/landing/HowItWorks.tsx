    ...
    <section id="how" className="py-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h2 className="mt-4 text-4xl md:text-5xl font-bold text-indigo-900 tracking-tight">7 simple steps to a filed return.</h2>
        ...
        <ol className="relative grid grid-cols-1 md:grid-cols-7 gap-8 md:gap-2">
          {STEPS.map((s, i) => (
            <li key={s.title} className="relative flex flex-col items-center text-center">
              {i < STEPS.length - 1 && (
                <div className="hidden md:block absolute top-7 left-1/2 w-full h-0.5 bg-gradient-to-r from-indigo-900 to-indigo-700" aria-hidden />
              )}
              <div className="relative z-10 h-14 w-14 rounded-full bg-indigo-600 text-white inline-flex items-center justify-center font-bold shadow-md ring-4 ring-indigo-900">
                {i + 1}
              </div>
              ...
              <p className="mt-1 text-sm text-slate-900 leading-relaxed">{s.desc}</p>
            </li>
          ))}
        </ol>
      </div>
