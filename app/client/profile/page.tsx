...
import { me, updateClientProfile, getOnboardingForm, submitOnboardingForm } from '@/lib/api';
          ...
          {fields.map((f) => (
            <div key={f.id}>
              ...
              {f.field_type === 'DATE' ? (
                ...
                <Input value={values[f.field_key] || ''} onChange={(e) => setValues({ ...values, [f.field_key]: e.target.value })} className="mt-1.5" />
              )}
            </div>
          ))}
