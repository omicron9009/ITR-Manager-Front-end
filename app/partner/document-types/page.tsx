...
function TextFieldTypesPanel() {
  ...
  const [form, setForm] = useState<{
    ...
    display_order: number;
    is_active: boolean;
  }>({ name: '', description: '', max_length: 200, display_order: 0, is_active: true });

  const load = async () => {
    setLoading(true);
    try {
      const r = await listTextFieldTypes(showAll);
      setTypes(r?.items ?? r ?? []);
    } catch {
      setTypes([]);
    } finally { setLoading(false); }
  ...
  const resetForm = () => setForm({ name: '', description: '', max_length: 200, display_order: 0, is_active: true });
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <Card className="rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-slate-500">No text fields found.</p>
          <Button onClick={openCreate} variant="outline" className="mt-4">
