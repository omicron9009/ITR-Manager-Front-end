...
export default function FormBuilderPage() {
  ...
  const load = async () => { try { const r = await listFields(); setFields(r?.items || r?.fields || r || []); } catch {} };
  useEffect(() => { load(); }, []);

  const save = async () => {
    try {
      const body: any = { label: newField.label, field_type: newField.field_type };
      if (newField.field_type === 'DROPDOWN') body.options = newField.options.split('\n').map((o) => o.trim()).filter(Boolean);
      ...
      <div className="lg:col-span-2 space-y-3">
        ...
        {adding && (
          <Card className="rounded-xl p-5 border-indigo-200 bg-indigo-50/30">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div><Label>Label</Label><Input value={newField.label} onChange={(e) => setNewField({ ...newField, label: e.target.value })} className="mt-1.5" /></div>
        ...
        {fields.length === 0 ? <Card className="rounded-xl p-10 text-center text-sm text-slate-500">No fields yet. Add your first onboarding field.</Card> : fields.map((f) => (
          <Card key={f.id} className="rounded-xl p-4 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3"><GripVertical className="h-4 w-4 text-slate-300 cursor-grab" /><div><div className="font-semibold text-slate-900">{f.label}</div><div className="text-xs text-slate-500">{f.field_type}{f.required ? ' · required' : ''}</div></div></div>
