...
function getRoleFromPath(pathname: string): 'partner' | 'executive' {
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="space-y-2">
        {items.map((n: any) => {
          ...
          return (
          <Card key={n.id} onClick={async () => { if (!n.is_read) await markRead([n.id]); const role = getRoleFromPath(pathname); const route = await resolveRoute(n, role); if (route) router.push(route); }} className={`rounded-xl p-4 transition-colors ${hasLink ? 'cursor-pointer hover:shadow-md' : ''} ${n.is_read ? 'bg-white' : 'bg-indigo-50/40 border-indigo-100'}`}>
